...
uint64_t smp_board_serial(void)
{
    ...
    serial = ((uint64_t)fuse->uid_high << 32) | fuse->uid_low;

    return serial;
}

bool smp_is_dev_board(void)
{
    uint32_t i;
    uint64_t uid;   
    bool is_dev = false;
    ...
    for (i = 0 ; i < ARRAY_SIZE(dev_board_uid_list); i++) {
...
int smp_stats_add_hex(const char *descr, uint32_t level, const char *field, uint32_t value) 
{
    char line[256];    
    snprintf(line, sizeof(line), "{\"name\":\"%s\",\"value\":0x%x,\"level\":%u,\"descr\":\"%s\"},", field, value, level, descr);  
    return (smp_stats_append(line));    
}
...
int smp_stats_flush(void)
{
    ...
    if (fs_set_blk_dev(FILEDATA_IFACE, FILEDATA_PART, FS_TYPE_EXT))
        return 1;
...
int smp_log_flush(void) 
{
    ...
    if (ret >= 0) {       
        if ((len_read == size) && !strncmp(readback_buffer, log_buffer, len_read)) {                  
            result = 0;
            goto end;
        }
    }
